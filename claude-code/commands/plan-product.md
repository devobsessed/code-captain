...
- Use `Glob` to scan existing `.code-captain/product/` for any existing product documentation
- Use `Read` to load available project context from `.code-captain/docs/` (tech-stack.md if available)
...
Use `Bash` to create directory structure, then `Write` to create each file.
...
- `/research` skill to investigate any market or product unknowns
```
...
- **Competition analysis** - Can explore competitive landscape using `WebSearch`
...
**Primary tools:**
- `TodoWrite` - Progress tracking throughout discovery and creation
- `Glob` - Locating existing product documentation
- `Read` - Loading project context and existing plans
- `Write` - Creating product planning documents
- `WebSearch` - Market research and competitive analysis (if needed)
...
- Can invoke `/research` skill for market or technical investigation
