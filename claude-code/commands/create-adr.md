...
2. If no research found: **automatically invoke the `/research` skill** to conduct complete research
...
1. **Check for existing research:**
   ...
   - Use `Glob` to look for related research in `.code-captain/research/` directory
   - Use `Bash` to list the research directory structure
...
2. **Automatic research execution if missing:**
   ...
   Invoking /research skill and executing complete research process..."
   ```

3. **Execute research workflow automatically:**

   - **Invoke the `/research` skill** with the decision topic as the argument
   - The research skill will execute the complete 4-phase research methodology:
     - Phase 1: Define Research Scope
     - Phase 2: Initial Discovery
     - Phase 3: Deep Dive Analysis
     - Phase 4: Synthesis and Recommendations
   - Research document will be created in `.code-captain/research/{date}-{topic}-research.md`
   - **ONLY CONTINUE** with ADR creation after research is completed

4. **Handle research workflow:**
   - If no research found: **AUTOMATICALLY invoke `/research` skill first**
...
Use `TodoWrite` to track the ADR creation process:
...
2. **Find existing ADRs and documentation:**
   ...
   - Use `Glob` to find existing ADRs in `.code-captain/decision-records/` directory
   - Use `Bash` to explore system structure and identify affected components
...
1. **Leverage existing research (if found in Step 0):**
   ...
   - Use `Read` to load research documents from `.code-captain/research/` for relevant findings
...
3. **Use `Grep` to understand current implementation approaches**
...
2. **Determine ADR number:**
   ...
   - Check existing ADRs in `.code-captain/decision-records/` directory using `Bash`
   - Use sequential numbering (0001, 0002, etc.)

3. **Create ADR directory structure** using `Bash` if needed
